...
pub enum Error {
    ...
    #[error(
        "database path not set: tests must set RQMD_INDEX_PATH or use Store::open() with an explicit path"
    )]
    DbPathNotSet,
}
...
impl Store {
    ...
    ///
    /// Respects `RQMD_INDEX_PATH` and the production-mode guard — see
