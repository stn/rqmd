...
use rqmd_core::{find_local_config_path, local_db_path};
use tempfile::TempDir;

#[test]
fn finds_local_config_from_nested_directory() {
    let root = TempDir::new().unwrap();
    let rqmd_dir = root.path().join(".rqmd");
    fs::create_dir_all(&rqmd_dir).unwrap();
    let config_path = rqmd_dir.join("index.yaml");
...
fn prefers_yaml_over_yml_when_both_exist() {
    ...
    let yml = rqmd_dir.join("index.yml");
...
fn returns_none_when_no_local_config_present() {
    ...
    // Walks all the way up to the filesystem root without finding `.rqmd`.
    // (Even if the user happens to have a `.rqmd` somewhere up the tree on
...
fn local_db_path_is_sibling_of_config() {
    ...
    assert_eq!(local_db_path(&config_path), rqmd_dir.join("index.sqlite"));
}
