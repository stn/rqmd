...
use rqmd_core::llm::traits::{LlamaToken, Llm};
...
impl Llm for MockLlm {
    ...
    ) -> rqmd_core::llm::error::Result<Option<GenerateResult>> {
    ...
    async fn model_exists(&self, name: &str) -> rqmd_core::llm::error::Result<ModelInfo> {
        Ok(ModelInfo {
            name: name.into(),
            exists: true,
            path: None,
    ...
    ) -> rqmd_core::llm::error::Result<Vec<Queryable>> {
    ...
    async fn rerank(
        ...
        docs: &[RerankDocument],
        _opts: RerankOptions,
    ) -> rqmd_core::llm::error::Result<RerankResult> {
    ...
    async fn tokenize(&self, text: &str) -> rqmd_core::llm::error::Result<Vec<LlamaToken>> {
    ...
    async fn detokenize(&self, tokens: &[LlamaToken]) -> rqmd_core::llm::error::Result<String> {
