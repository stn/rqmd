//! [`RmdStore`] — the top-level handle external Rust callers use.
//!
...
//! # Lifecycle
//!
//! Rust has no async `Drop`, so callers MUST invoke [`RmdStore::close`] (or
//! [`RmdStore::shutdown`] for `Arc<Mutex<RmdStore>>` patterns) before the
...
/// Errors produced by [`RmdStore`] methods.
///
...
/// Construction options for [`RmdStore::open`].
///
...
/// Options for the unified [`RmdStore::search`] entry point.
///
...
/// Aggregate result from [`RmdStore::update`].
...
/// Options for [`RmdStore::add_collection`].
#[derive(Debug, Default, Clone)]
...
impl RmdStore {
    ...
    pub fn open(opts: RmdStoreOptions) -> Result<Self> {
    ...
    /// write-through that [`RmdStore`] mutators provide. Use only for
