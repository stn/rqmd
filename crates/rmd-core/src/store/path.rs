...
//! `RMD_INDEX_PATH` is set — this prevents tests from accidentally writing
...
///
/// 1. `RMD_INDEX_PATH` env var — always honoured.
...
pub fn default_db_path(index_name: Option<&str>) -> Result<PathBuf> {
    if let Some(p) = std::env::var_os("RMD_INDEX_PATH")
        && !p.is_empty()
    {
...
/// Home directory (delegates to [`crate_paths::rmd_homedir`]). Matches
/// TS `homedir()` (`store.ts:339`) within the renamed `rmd` namespace.
pub fn homedir() -> PathBuf {
    crate_paths::rmd_homedir()
}
