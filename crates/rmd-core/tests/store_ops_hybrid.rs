...
use rmd_core::llm::types::{QueryType, Queryable};
...
async fn vector_search_query_drops_lex_expansions() {
    ...
    for txt in [
        rmd_core::llm::format::format_query_for_embedding("alpha", "m"),
        rmd_core::llm::format::format_query_for_embedding("vec variation", "m"),
    ] {
...
async fn structured_search_first_list_gets_2x_weight() {
    ...
    let mock = Arc::new(MockLlm::new(4));
    mock.set_embed(
        rmd_core::llm::format::format_query_for_embedding("gamma topic", "m"),
        vec![0.0, 0.0, 1.0, 0.0],
    );
