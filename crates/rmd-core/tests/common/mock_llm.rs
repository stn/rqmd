...
use rmd_core::llm::traits::{LlamaToken, Llm};
use rmd_core::llm::types::{
...
impl Llm for MockLlm {
    async fn embed(&self, text: &str, _opts: EmbedOptions) -> rmd_core::llm::error::Result<Option<EmbeddingResult>> {
    ...
    async fn embed_batch(
        ...
        texts: &[String],
        opts: EmbedOptions,
    ) -> rmd_core::llm::error::Result<Vec<Option<EmbeddingResult>>> {
    ...
    ) -> rmd_core::llm::error::Result<Option<GenerateResult>> {
    ...
    async fn model_exists(&self, name: &str) -> rmd_core::llm::error::Result<ModelInfo> {
        Ok(ModelInfo {
            name: name.into(),
            exists: true,
            path: None,
    ...
    ) -> rmd_core::llm::error::Result<Vec<Queryable>> {
    ...
    async fn rerank(
        ...
        docs: &[RerankDocument],
        _opts: RerankOptions,
    ) -> rmd_core::llm::error::Result<RerankResult> {
    ...
    async fn tokenize(&self, text: &str) -> rmd_core::llm::error::Result<Vec<LlamaToken>> {
    ...
    async fn detokenize(&self, tokens: &[LlamaToken]) -> rmd_core::llm::error::Result<String> {
