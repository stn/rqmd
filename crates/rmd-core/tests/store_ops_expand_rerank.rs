...
use rmd_core::store::cache::{get_cache_key, get_cached_result};
use rmd_core::store::Store;
use rmd_core::store_ops::{expand_query, rerank, ExpandedQueryType, RerankCandidate};
...
async fn expand_query_migrates_legacy_text_format() {
    ...
    store
        .with_connection(|c| {
            rmd_core::store::cache::set_cached_result(
                c,
                &key,
