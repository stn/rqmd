...
use crate::prompt;
use crate::pull;
use crate::traits::{Llm, LlamaToken};
use crate::types::{
...
use crate::worker::{EmbedPool, EmbedWorker, RerankPool, RerankWorker, split_into_chunks};
    ...
    async fn dispose(&self) {
        ...
        while self.in_flight.count() > 0 {
            if start.elapsed() > drain_timeout {
                tracing::warn!(
                    "rmd-llm dispose: {} in-flight op(s) did not drain in 30s; proceeding",
                    self.in_flight.count()
                );
                break;
            }
        ...
        if let Some(pool_arc) = self.embed_pool.swap(None) {
            match Arc::try_unwrap(pool_arc) {
                ...
                Err(_) => {
                    tracing::warn!(
                        "rmd-llm dispose: embed pool still has Arc holders; workers will be \
        ...
        if let Some(pool_arc) = self.rerank_pool.swap(None) {
            match Arc::try_unwrap(pool_arc) {
                ...
                Err(_) => {
                    tracing::warn!(
                        "rmd-llm dispose: rerank pool still has Arc holders; workers will be \
                         cleaned up by Drop instead of explicit join"
                    );
                }
            }
...
async fn join_embed_worker_with_timeout(worker: EmbedWorker, index: usize, timeout: Duration) {
    ...
    if tokio::time::timeout(timeout, join).await.is_err() {
        tracing::warn!(
            "rmd-llm dispose: embed worker #{index} did not join within {timeout:?}; \
             leaking the thread"
        );
    }
}
...
async fn join_rerank_worker_with_timeout(worker: RerankWorker, index: usize, timeout: Duration) {
    ...
    if tokio::time::timeout(timeout, join).await.is_err() {
        tracing::warn!(
            "rmd-llm dispose: rerank worker #{index} did not join within {timeout:?}; \
             leaking the thread"
        );
    }
}
...
fn flatten_with_chunk_fallback<T>(
    ...
    raw.into_iter()
        ...
        .flat_map(|(result, size)| match result {
            ...
            Err(e) => {
                tracing::warn!(
                    "rmd-llm: chunk of {size} input(s) failed and will be reported as \
                     None slots: {e}"
                );
