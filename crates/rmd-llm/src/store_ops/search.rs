...
//! [`rmd_core::store::embeddings::search_vec_with_embedding`].
//!
//! Port of `tobi/qmd/src/store.ts` lines 3252–3349 (the LLM half;
//! the pure-SQL half is in rmd-core).

use std::sync::Arc;

use rmd_core::store::embeddings::search_vec_with_embedding;
use rmd_core::store::search::SearchResult;
use rmd_core::store::Store;

use crate::format::{format_doc_for_embedding, format_query_for_embedding};
