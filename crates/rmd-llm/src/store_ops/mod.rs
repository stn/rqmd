...
//! ([`rmd_core::store::embeddings`]) and status types
//! ([`rmd_core::store::status`]). This module composes those with the
...
pub use rmd_core::store::embeddings::{EmbeddingDoc, HashForEmbedding, PendingEmbeddingDoc};
pub use rmd_core::store::status::{IndexHealthInfo, IndexStatus};
...
/// Errors produced by orchestration functions. Wraps both rmd-core and
/// rmd-llm errors so callers don't have to convert manually, and adds
/// orchestrator-specific variants (vec table missing, session timeout
/// observed mid-run, malformed structured search input).
#[derive(Debug, thiserror::Error)]
pub enum Error {
    #[error(transparent)]
    Core(#[from] rmd_core::store::Error),

    #[error(transparent)]
    Llm(#[from] crate::Error),
