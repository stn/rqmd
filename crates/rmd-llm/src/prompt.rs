...
///
/// See `crates/rmd-llm/examples/spike_03_rerank.rs` for the measurement
...
pub fn parse_expand_query_output(raw: &str) -> Vec<crate::types::Queryable> {
    let mut out = Vec::new();
    for line in raw.lines() {
        let line = line.trim_start();
        if let Some(rest) = line.strip_prefix("lex:") {
            out.push(make_queryable(crate::types::QueryType::Lex, rest));
        } else if let Some(rest) = line.strip_prefix("vec:") {
            out.push(make_queryable(crate::types::QueryType::Vec, rest));
        } else if let Some(rest) = line.strip_prefix("hyde:") {
            out.push(make_queryable(crate::types::QueryType::Hyde, rest));
        }
    }
    out
}

fn make_queryable(type_: crate::types::QueryType, rest: &str) -> crate::types::Queryable {
...
pub fn filter_with_query_terms(
    query: &str,
    candidates: Vec<crate::types::Queryable>,
) -> Vec<crate::types::Queryable> {
...
pub fn fallback_queryables(query: &str, include_lexical: bool) -> Vec<crate::types::Queryable> {
    let mut out = vec![
        crate::types::Queryable {
            type_: crate::types::QueryType::Hyde,
            text: format!("Information about {query}"),
        },
        crate::types::Queryable {
            type_: crate::types::QueryType::Lex,
            text: query.to_owned(),
        },
        crate::types::Queryable {
            type_: crate::types::QueryType::Vec,
            text: query.to_owned(),
        },
    ];
    if !include_lexical {
        out.retain(|q| q.type_ != crate::types::QueryType::Lex);
    }
    out
}
