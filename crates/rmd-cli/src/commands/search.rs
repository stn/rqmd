//! `rmd search` — BM25 full-text search. Sync because no LLM is involved.
//!
//! Maps to qmd's `search` CLI handler (`src/cli/qmd.ts` lines 2392–2426).

use anyhow::{bail, Result};
use rmd_core::store::search::search_fts;
...
pub fn run(args: SearchArgs, state: &mut IndexState, p: &Palette) -> Result<()> {
    ...
    // None path, so we deliberately do NOT bail — `rmd search "$VAR" --json`
