//! `rmd get` — show a single document.
//!
//! Maps to qmd's `qmd get` in `src/cli/qmd.ts` (lines 919–1103, 3369–3378).
//! Most resolution work lives in `rmd_core::store::lookup::find_document`.

use anyhow::{Result, anyhow};
use rmd_core::store::lookup::{FindDocumentOptions, FindDocumentOutcome, find_document};
use rmd_core::store::snippet::add_line_numbers;
