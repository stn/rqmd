//! `rmd context ...` — attach human-written context summaries.
//!
...
use rmd_core::store::path::{homedir, pwd, real_path};
use rmd_core::store::virtual_path::{is_virtual_path, parse_virtual_path};
...
fn add(a: ContextAddArgs, state: &mut IndexState, p: &Palette) -> Result<()> {
    ...
    if text.is_empty() {
        return Err(anyhow!(
            "Usage: rmd context add [path] \"text\"\n\n\
             Examples:\n  \
             rmd context add \"Context for current directory\"\n  \
             rmd context add . \"Context for current directory\"\n  \
             rmd context add qmd://notes/ \"Context for entire notes collection\"\n  \
             rmd context add / \"Global context for all collections\""
        ));
    }
...
fn list(state: &mut IndexState, p: &Palette) -> Result<()> {
    ...
    if entries.is_empty() {
        println!(
            "{}No contexts configured. Use 'rmd context add' to add one.{}",
            p.dim(),
            p.reset()
        );
        return Ok(());
