//! `rmd ls` — list collections, or files within a collection.
//!
//! Maps to qmd's `qmd ls` in `src/cli/qmd.ts` (lines 1340–1498).

use anyhow::{anyhow, Result};
use rmd_core::db::params;
use rmd_core::store::context::list_collections;
use rmd_core::store::virtual_path::parse_virtual_path;
...
pub fn run(a: LsArgs, state: &mut IndexState, p: &Palette) -> Result<()> {
    ...
    let Some(arg) = a.path else {
        ...
        if collections.is_empty() {
            println!("No collections found. Run 'rmd collection add .' to index files.");
            return Ok(());
        }
        println!("{}Collections:{}\n", p.bold(), p.reset());
    ...
    if !collections.iter().any(|c| c.name == collection_name) {
        return Err(anyhow!(
            "Collection not found: {collection_name}\nRun 'rmd ls' to see available collections."
        ));
    }
    ...
    let files: Vec<(String, String, String, i64)> = store.with_connection(|conn| {
        ...
        Ok::<_, rmd_core::store::Error>(rows)
    })?;
...
fn resolve_collection_arg(
    arg: &str,
    known: &[rmd_core::store::context::CollectionListing],
...
fn longest_prefix_match<'a>(
    s: &str,
    known: &'a [rmd_core::store::context::CollectionListing],
) -> Option<&'a str> {
    known
        .iter()
